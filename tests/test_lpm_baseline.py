...
from pert_gym.models import LatentPerturbationModel


def test_latent_perturbation_model_runs_tiny_synthetic_smoke() -> None:
    ...
    assert model.perturbation_deltas_ is not None
    assert set(model.perturbation_deltas_) == {"drug_a", "drug_b"}


def test_latent_perturbation_model_runs_model_ready_v0_metadata_smoke() -> None:
...
def test_latent_perturbation_model_rejects_control_only_training() -> None:
    model = LatentPerturbationModel(epochs=1)
