...
def matrix_for(n_obs: int, sparse_format: str):
    ...
    matrix = sparse.random(n_obs, n_vars, density=density, format=sparse_format, random_state=20260711 + n_obs, dtype=np.float32)
    matrix.sum_duplicates()
    matrix.sort_indices()
    return matrix


def write_and_readback(root: Path, matrix, sparse_format: str, *, source_row_start: int) -> dict[str, object]:
    ...
    group.attrs.update({"format": sparse_format, "shape": list(matrix.shape), "nnz": matrix.nnz})
    group.create_dataset("data", data=matrix.data, chunks=(min(matrix.nnz, 65_536),))
    group.create_dataset("indices", data=matrix.indices, chunks=(min(matrix.nnz, 65_536),))
    group.create_dataset("indptr", data=matrix.indptr, chunks=(min(len(matrix.indptr), 65_536),))
    obs_index = np.arange(matrix.shape[0], dtype=np.int64)
    source_rows = np.arange(source_row_start, source_row_start + matrix.shape[0], dtype=np.int64)
    (root / "obs.json").write_text(json.dumps({"index": obs_index.tolist(), "source_rows": source_rows.tolist()}))

    loaded = zarr.open_group(str(root), mode="r")
    constructor = sparse.csr_matrix if sparse_format == "csr" else sparse.csc_matrix
    roundtrip = constructor((np.asarray(loaded["data"]), np.asarray(loaded["indices"]), np.asarray(loaded["indptr"])), shape=tuple(loaded.attrs["shape"]))
    obs_sidecar = json.loads((root / "obs.json").read_text())
    return {
        "write_readback_seconds": time.perf_counter() - started,
        "bytes": stored_bytes(root),
        "matrix_parity": roundtrip.shape == matrix.shape and roundtrip.nnz == matrix.nnz and np.array_equal(roundtrip.data, matrix.data) and np.array_equal(roundtrip.indices, matrix.indices) and np.array_equal(roundtrip.indptr, matrix.indptr),
...
def run_case(root: Path, n_obs: int, sparse_format: str) -> dict[str, object]:
    """Measure generation plus write/readback with an explicit case-local RSS delta."""
    started = time.perf_counter()
    baseline_rss = local_rss_bytes()
    matrix = matrix_for(n_obs, sparse_format)
    peak_rss = max(baseline_rss, local_rss_bytes())
    result = write_and_readback(root, matrix, sparse_format, source_row_start=0)
    peak_rss = max(peak_rss, local_rss_bytes())
    result.update({
        "n_obs": n_obs,
...
def main() -> int:
    ...
    if "pert-gym-worker-eu" not in host:
        raise RuntimeError(f"VM-only benchmark refused on host {host!r}; expected pert-gym-worker-eu")

    base = args.workdir or Path(tempfile.mkdtemp(prefix="pert-gym-sparse-zarr-"))
    created_base = args.workdir is None
    total_started = time.perf_counter()
    try:
        cases = [run_case(base / f"{sparse_format}-{n_obs}", n_obs, sparse_format) for n_obs in (5_000, 10_000, 25_000) for sparse_format in ("csr", "csc")]
        if not all(case["matrix_parity"] and case["obs_parity"] and case["source_row_parity"] for case in cases):
            raise RuntimeError("sparse Zarr benchmark parity failed")
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(json.dumps({"host": host, "total_wall_seconds": time.perf_counter() - total_started, "cases": cases}, indent=2) + "\n")
    finally:
        if created_base:
