...
DATASETS: tuple[DatasetPlan, ...] = (
    ...
    DatasetPlan(
        name="PRoPER-seq / ProPer-seq 2026",
        family="probe-based Perturb-seq",
        modality="scRNA-seq",
        perturbation_axis="source TBD",
        priority=2,
        lamin_prefix="properseq",
        local_dir="source TBD",
        ingestion_entrypoint="tools.ingest_phase3_scrna.ingest_properseq",
        status="blocked_source_tbd_legacy_excluded",
        expected_outputs=["obs.parquet", "X.h5ad", "var.parquet"],
        next_action="Locate the real 2026 probe-based Perturb-seq scRNA expression matrix before writing any artifacts.",
        notes="The legacy GSE150818 chimeric-read-pair dataset is excluded and archived from the active jkobject branch if present.",
    ),
    DatasetPlan(
