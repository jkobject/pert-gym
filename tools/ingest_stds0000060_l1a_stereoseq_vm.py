...
TASK_ID = "t_542243b8"
URL = "https://ftp.cngb.org/pub/SciRAID/stomics/STDS0000060/stomics/L1_a_count_normal_stereoseq.h5ad"
FILENAME = "L1_a_count_normal_stereoseq.h5ad"
PREFIX = "temporal_pretraining/stomics/STDS0000060_drosophila_stereoseq/L1_a_count_normal_stereoseq"
PRIOR_PREFIX = "temporal_pretraining/stomics/STDS0000060_drosophila_stereoseq/E14-16h_a_count_normal_stereoseq"
ARTIFACT_JSON = ROOT / "artifacts/schema_audit/stds0000060_L1_a_lamin_ingestion_20260703.json"
ARTIFACT_MD = ROOT / "artifacts/schema_audit/stds0000060_L1_a_lamin_ingestion_20260703.md"
EXPECTED_HOST = "pert-gym-worker-eu"
EXPECTED_LENGTH = 1864536390
...
def enrich_obs(obs: pd.DataFrame) -> pd.DataFrame:
    ...
    obs["sample"] = "L1_a_count_normal_stereoseq"
    obs["developmental_stage"] = "L1"
...
def write_triplet(ln: Any, h5ad_path: Path, obs: pd.DataFrame, var: pd.DataFrame) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="stds0000060_l1a_") as td:
...
def write_reports(out: dict[str, Any]) -> None:
    ...
    lines = [
        "# STDS0000060 L1_a Stereo-seq Lamin ingestion report",
        "",
        ...
        f"- prior E14-16h untouched before: `{out.get('prior_existing_before')}`",
        f"- target after: `{out.get('target_existing_after')}`",
        f"- prior E14-16h untouched after: `{out.get('prior_existing_after')}`",
        "",
        "## Smoke",
        "```json",
...
def main() -> None:
    ...
    p.add_argument("--work-dir", type=Path, default=Path("data/vm_stage/stds0000060/L1_a"))
    ...
    if args.write:
        ln.track(path="tools/ingest_stds0000060_l1a_stereoseq_vm.py")
    ...
    out: dict[str, Any] = {
        ...
        "prior_existing_before": duplicate_keys(ln, PRIOR_PREFIX),
    }
    ...
    if args.verify_only:
        ...
        out["prior_readback"] = verify_prefix(ln, PRIOR_PREFIX)
        out["target_existing_after"] = duplicate_keys(ln, PREFIX)
        out["prior_existing_after"] = duplicate_keys(ln, PRIOR_PREFIX)
