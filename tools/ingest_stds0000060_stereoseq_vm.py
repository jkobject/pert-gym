...
TASK_ID = "t_74356e52"
URL = "https://ftp.cngb.org/pub/SciRAID/stomics/STDS0000060/stomics/E14-16h_a_count_normal_stereoseq.h5ad"
FILENAME = "E14-16h_a_count_normal_stereoseq.h5ad"
PREFIX = "temporal_pretraining/stomics/STDS0000060_drosophila_stereoseq/E14-16h_a_count_normal_stereoseq"
E16_PREFIX = "temporal_pretraining/stomics/STDS0000060_drosophila_stereoseq/E16-18h_a_count_normal_stereoseq"
ARTIFACT_JSON = ROOT / "artifacts/schema_audit/stds0000060_E14-16h_lamin_ingestion_20260703.json"
ARTIFACT_MD = ROOT / "artifacts/schema_audit/stds0000060_E14-16h_lamin_ingestion_20260703.md"
EXPECTED_HOST = "pert-gym-worker-eu"
EXPECTED_LENGTH = 1675109862
...
def enrich_obs(obs: pd.DataFrame) -> pd.DataFrame:
    ...
    obs["sample"] = "E14-16h_a_count_normal_stereoseq"
    obs["developmental_stage"] = "E14-16h"
...
def write_triplet(ln: Any, h5ad_path: Path, obs: pd.DataFrame, var: pd.DataFrame) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="stds0000060_e14_") as td:
...
def write_reports(out: dict[str, Any]) -> None:
    ...
    lines = [
        "# STDS0000060 E14-16h Stereo-seq Lamin ingestion report",
        "",
        ...
        f"- E16 untouched before: `{out.get('e16_existing_before')}`",
        f"- target after: `{out.get('target_existing_after')}`",
        f"- E16 untouched after: `{out.get('e16_existing_after')}`",
        "",
        "## Smoke",
        "```json",
...
def main() -> None:
    ...
    p.add_argument("--work-dir", type=Path, default=Path("data/vm_stage/stds0000060/E14-16h"))
    ...
    if args.write:
        ln.track(path="tools/ingest_stds0000060_stereoseq_vm.py")
    ...
    out: dict[str, Any] = {
        ...
        "e16_existing_before": duplicate_keys(ln, E16_PREFIX),
    }
    ...
    if args.verify_only:
        ...
        out["e16_readback"] = verify_prefix(ln, E16_PREFIX)
        out["target_existing_after"] = duplicate_keys(ln, PREFIX)
        out["e16_existing_after"] = duplicate_keys(ln, E16_PREFIX)
