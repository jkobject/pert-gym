...
Guarded for Kanban t_a129af80. Runs only on pert-gym-worker-eu. Current tranche
is exactly one already-staged processed H5AD, E16.5_E1S3_cell_bin_whole_brain,
from the reviewed 20260704 MOSTA/STDS0000058 source manifest.
"""
...
REPORT_JSON = (
    ROOT
    / f"artifacts/schema_audit/mosta_E16p5_E1S3_cell_bin_whole_brain_tranche_{DATE}.json"
)
REPORT_MD = (
    ROOT
    / f"artifacts/schema_audit/mosta_E16p5_E1S3_cell_bin_whole_brain_tranche_{DATE}.md"
)

TRANCHE = [
    {
        "sample": "E16.5_E1S3_cell_bin_whole_brain",
        "stage": "E16.5",
        "section": "E1S3_cell_bin_whole_brain",
        "source_uri": "gs://scperturb/pert-gym/staging/temporal_pretraining/mosta_stds0000058_20260704/stomics/E16.5_E1S3_cell_bin_whole_brain.h5ad",
        "prefix": "temporal_pretraining/mosta_stds0000058/E16.5_E1S3_cell_bin_whole_brain",
        "expected_gcs_size_bytes": 732847864,
        "local_source": "/tmp/pert-gym-mosta-tranche/E16.5_E1S3_cell_bin_whole_brain.h5ad",
    }
]
...
def choose_x(source: ad.AnnData) -> tuple[Any, dict[str, Any]]:
    if "count" in source.layers or "counts" in source.layers:
        layer_name = "count" if "count" in source.layers else "counts"
        x = source.layers[layer_name]
        semantics = "raw_counts_from_count_layer"
        source_matrix = f"layers[{layer_name!r}]"
    else:
        x = source.X
...
def write_reports(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# MOSTA E16.5_E1S3_cell_bin_whole_brain tranche report",
        "",
        ...
        "",
        "## Scope",
        "- One already-staged processed H5AD: `E16.5_E1S3_cell_bin_whole_brain.h5ad`.",
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "tranche": TRANCHE,
        "commands": [
            "uv run python tools/ingest_mosta_e16p5_e1s3_cell_bin_whole_brain_tranche.py --dry-run",
            "uv run python tools/ingest_mosta_e16p5_e1s3_cell_bin_whole_brain_tranche.py --write",
            "uv run python tools/ingest_mosta_e16p5_e1s3_cell_bin_whole_brain_tranche.py --verify-only",
        ],
        "lamin": {
    ...
    if not args.verify_only:
        ln.track(path="tools/ingest_mosta_e16p5_e1s3_cell_bin_whole_brain_tranche.py")
