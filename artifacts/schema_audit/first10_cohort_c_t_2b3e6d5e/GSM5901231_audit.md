# GSM5901231 scientific audit and correction plan
...
- GEO child: [GSM5901231](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM5901231&targ=self&form=text&view=full) of [GSE196799](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE196799&targ=self&form=text&view=full).
...
- OBS: Artifact `HAjNzeHiAFVj18Bl0000` / `data/cleaned/GSM5901231/obs.parquet` / Lamin hash `bViAo4Hii_M-eDkmNYL-mw` / GCS generation `1785154234957959` / payload SHA-256 `471a5f6bdbc6430a46b882f7fd38642d3d8680575f69bbdd059e44c785679352`.
- X: Artifact `DMOKXzr7fGa2BzFx0000` / `data/cleaned/GSM5901231/X.h5ad` / Lamin hash `EjV7qoYehJmuVKTf_aVjYw` / GCS generation `1785154234986151` / payload SHA-256 `45c310d2d01c2670078ea9deffbe60ee2b46e4b72fbe520b8e6fdb0ac170a212`.
- VAR: Artifact `2L5w9PF2KfG568jw0000` / `data/cleaned/GSM5901231/var.parquet` / Lamin hash `5IxJsE91crLE_PZZ7Ccq1w` / GCS generation `1785154234984134` / payload SHA-256 `1d74fc35a8ed61c7f487b5b3b955fd91e9208e6e42e7943ebf0e8442ec83c134`.

## OBS column audit

Current OBS has 6,430 rows, unique ordered index SHA-256 `cb4fa0cbd90407a4d00c3f479140ce55dac9867634ac385677dc27aca998c0ee`, and no nulls in any current column. It lacks required `obs_uuid` and `original_obs_index`.

| column | dtype | cardinality | missing | decision | target | rationale |
|---|---|---:|---:|---|---|---|
| `cell_id` | `object` | 6430 | 0 | `keep` | `cell_id` | unique source-backed cell identity |
| `barcode` | `object` | 6430 | 0 | `keep` | `barcode` | unique source 10x barcode; useful for replay |
...
| `n_genes_by_counts` | `int32` | 3168 | 0 | `map` | `n_genes` | cell-level QC covariate |
| `total_counts` | `int64` | 5489 | 0 | `map` | `n_counts` | cell-level QC covariate |
| `pct_counts_mt` | `float32` | 6420 | 0 | `map` | `pct_mito` | cell-level QC covariate |
...
This child is one snapshot at day 9 (12960 minutes), so its child verdict is `non_temporal_single_snapshot` and the redundant per-cell time fields move to dataset metadata. GSE196799 remains `temporal_time_course` across days [0, 3, 6, 9, 12, 18]; the family relation is not erased.
...
- X: `csr_matrix` `int32` raw integer counts, shape [6430, 20631], nnz 26,039,746, sum 97,683,665; exact OBS/X and X/VAR axis parity passes.
- Links: OBS→X `True` targeting `DMOKXzr7fGa2BzFx0000`; X→VAR `True` targeting `2L5w9PF2KfG568jw0000`.
