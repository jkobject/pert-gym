...
EXPECTED_PREWRITE_VAR_UID = "rRlvtvSEpbFnek7K0001"
...
PMC_TABLE_SPEC = (5_338_747, "fd307e6aec5a0044e0ec135594ed1d3071d3efb609809faa1f0ef91d111b465c")
...
AUTHOR_SPECS = {
    "unified_metadata.tsv.gz": (9_302_370, "c1ac69e9aa3f6bc3ed43df116ae125ab9cf7a9612456db579576a191aea8b68b"),
    "unified_display_names.tsv.gz": (439_619, "7bccfafa7ac9f6e01e8db7bdecb25e3f4afc52cabd343c9db5c4ab367ad605c5"),
}
...
CANONICAL_OBS_FIELDS = (
    ...
    "perturbation_library", "guide_sequence", "molecule_sequence", "is_control",
    "dose", "dose_unit", "timepoint", "trajectory_id", "pseudotime", "is_baseline",
    "sensitivity", "response_metric", "response_value", "response_source", "n_counts",
    "n_genes", "pct_mito", "pct_ribo", "is_low_quality",
)
NOT_APPLICABLE_FIELDS = {
    "molecule_sequence", "dose", "dose_unit", "sensitivity", "response_metric",
    "response_value", "response_source",
}
...
def download(url: str, path: Path, spec: tuple[int, str]) -> None:
    ...
    if not path.exists() or path.stat().st_size != size or sha256_file(path) != sha256:
        request = urllib.request.Request(url, headers={"User-Agent": "pert-gym-curation/1"})
...
def load_sources() -> dict[str, Any]:
    ...
    display = pd.read_csv(paths["unified_display_names.tsv.gz"], sep="\t", dtype="string")
    ...
    if len(genes) != EXPECTED_N_VARS or not genes.is_unique or features[1:].tolist() != genes.tolist():
        raise AssertionError("GEO expression feature-axis drift")
    if not display["feature_id"].is_unique or set(display["feature_id"].astype(str)) != set(genes.astype(str)):
    ...
    if set(table.columns) != {
        "Cell barcode", "Guide identity", "Guide target", "Library",
        "RNA velocity trajectory", "Gene expression UMI",
        "Gene expression complexity", "CRISPRi sgRNA UMI", "Protospacer", "Batch",
    }:
...
def exact_source_join(obs: pd.DataFrame, table: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    mismatches["sample"] = int((~expected_sample.eq(obs["sample"].astype("string"))).sum())
    source_control = joined["Guide target"].eq("Non-Targeting") & joined["Library"].eq("Control")
    mismatches["is_control"] = int((~source_control.astype(bool).eq(obs["is_control"].astype(bool))).sum())
    if any(mismatches.values()):
        raise AssertionError(f"source/current OBS semantic mismatch: {mismatches}")
    return joined, {
        "source_rows": len(table), "current_rows": len(obs), "identity_order_match": True,
...
def curate_obs(obs: pd.DataFrame, source: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    controls = joined["Guide target"].eq("Non-Targeting") & joined["Library"].eq("Control")
    sample = "diff_" + joined["Batch"].astype("string")
    set_field(curated, "dataset", PREFIX, "known", "canonical logical family")
    set_field(curated, "sample", sample, "known", "Table S5 batch mapped to source diff_N sample")
    set_field(curated, "cell_id", joined["Cell barcode"].astype("string"), "known", "Table S5 Cell barcode")
    set_field(curated, "batch", sample, "known", "Table S5 Batch")
    set_field(curated, "cell_type", "day 8 neural induction cell", "known", "GEO D8 and publication heterogeneous neural induction endpoint")
    set_field(curated, "cell_line", "CRISPRi WTC11 iPSC (Gen1C)", "known", "publication key resources and GEO iPSC WTC")
    set_field(curated, "disease", "non-diseased", "known", "publication experimental model")
    set_field(curated, "tissue_type", "in vitro cell culture", "known", "GEO experimental model")
    ...
    set_field(curated, "sequencer", "Illumina NovaSeq 6000", "known", "GEO Perturb-seq samples")
    set_field(curated, "technology", "10x Genomics Chromium 3' Assay v3 with direct sgRNA capture", "known", "GEO extraction protocol and publication")
    ...
    set_field(curated, "media", "dual SMAD-inhibition neural induction medium", "known", "GEO treatment protocol")
    set_field(curated, "is_bulk", False, "known", "single-cell source")
    set_field(curated, "is_pseudobulk", False, "known", "single-cell source")
    perturbation = joined["Guide target"].astype("string").replace({"Non-Targeting": "non-targeting"})
    set_field(curated, "perturbation", perturbation, "known", "Table S5 Guide target; canonical non-targeting control spelling")
    set_field(curated, "perturbation_type", "CRISPRi", "known", "publication experimental design")
    set_field(curated, "perturbation_technology", "CRISPR interference", "known", "publication experimental design")
    set_field(curated, "perturbation_library", "Wu et al. 492-sgRNA coding/lncRNA Perturb-seq library", "known", "publication and GEO sgRNA features")
    set_field(curated, "guide_sequence", joined["Protospacer"].astype("string"), "known", "Table S5 Protospacer")
    set_field(curated, "is_control", controls.astype("boolean"), "known", "Table S5 Non-Targeting target and Control library")
    set_field(curated, "timepoint", 11_520.0, "known", "GEO D8; minutes from neural-induction initiation")
    set_field(curated, "trajectory_id", joined["RNA velocity trajectory"].astype("string"), "known", "Table S5 RNA velocity trajectory")
    set_field(curated, "is_baseline", False, "known", "all accepted cells harvested at day 8")
    set_field(curated, "n_counts", pd.to_numeric(joined["Gene expression UMI"], errors="raise").astype("Int64"), "known", "Table S5 Gene expression UMI")
    set_field(curated, "n_genes", pd.to_numeric(joined["Gene expression complexity"], errors="raise").astype("Int64"), "known", "Table S5 Gene expression complexity")
    set_field(curated, "pct_mito", pd.to_numeric(original["percent_mito"], errors="raise").astype("Float64"), "known", "source Seurat percent_mito")
    set_field(curated, "pct_ribo", pd.to_numeric(original["percent_ribo"], errors="raise").astype("Float64"), "known", "source Seurat percent_ribo")
    set_field(curated, "is_low_quality", False, "known", "Table S5 filtered singlet barcode inclusion")
    ...
    curated["source_sgrna_umi"] = pd.to_numeric(joined["CRISPRi sgRNA UMI"], errors="raise").astype("Int64")
    ...
    return curated, {
        **join_receipt, "control_rows": int(controls.sum()),
        "guide_sequence_known_rows": int(curated["guide_sequence"].notna().sum()),
        "trajectory_known_rows": int(curated["trajectory_id"].notna().sum()),
        "qc_complete_rows": int(curated[["n_counts", "n_genes", "pct_mito", "pct_ribo", "is_low_quality"]].notna().all(axis=1).sum()),
    }
...
def field_dispositions(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    for field in CANONICAL_OBS_FIELDS:
        ...
        result[field] = {
            "disposition": disposition, "known_rows": known,
            "unknown_rows": len(frame) - known, "source_bound": disposition.startswith("materialized"),
        }
    return result
...
def reference_by_feature(source: dict[str, Any]) -> pd.DataFrame:
    ...
    result["source_reference_gene_ids"] = grouped.map(lambda values: "|".join(map(str, values))).astype("string")
    result["source_reference_gene_id_count"] = grouped.map(len).astype("Int64")
    result["source_native_gene_id"] = grouped.map(lambda values: values[0] if len(values) == 1 else pd.NA).astype("string")
...
def curate_var(var: pd.DataFrame, source: dict[str, Any]) -> pd.DataFrame:
    ...
    for column in ("stable_feature_id", "ensembl_gene_id", "stable_feature_id_source", "stable_feature_id_mapping_status"):
    ...
    curated["stable_feature_id_source"] = f"Wu et al. unified_metadata.tsv.gz at Git commit {AUTHOR_COMMIT}"
    ...
    curated["stable_feature_id_mapping_status"] = np.select(
        ...
        ["exact_source_ensembl_gene_id", "source_native_lh_id_no_ensembl", "ambiguous_source_gene_ids"],
        default="noncanonical_source_gene_id",
    )
    curated["stable_feature_id_candidate_count"] = curated["source_reference_gene_id_count"]
...
def verify_var(var: pd.DataFrame, source: dict[str, Any], x_axis: pd.Index) -> dict[str, Any]:
    ...
    if not var["feature_index"].astype("string").equals(pd.Series(var.index.astype(str), index=var.index, dtype="string")):
    ...
    if stable_count != EXPECTED_STABLE_ENSG_COUNT or lh_count != EXPECTED_SOURCE_LH_COUNT:
    ...
    return {
        ...
        "other_nonpassing_features": len(var) - stable_count - lh_count,
        "feature_index_unique": bool(var["feature_index"].is_unique),
        "axis_order_parity": True,
        "axis_order_sha256": ordered_sha256(var.index),
        "var_ensembl_species_completed": stable_count == len(var),
        "verdict": "false",
        "reason": "16401 source-native LH features and 71 ambiguous/noncanonical source features lack a unique standard ENSG; no identifier was fabricated",
    }
...
def x_axis(artifact: Any) -> tuple[pd.Index, dict[str, Any]]:
    ...
    return axis, {**EXPECTED_X, "shape": [EXPECTED_N_OBS, EXPECTED_N_VARS], "obs_names_sha256": obs_hash, "var_names_sha256": var_hash, "backed_only": True}
...
def collection_snapshot(ln: Any) -> dict[str, Any]:
    ...
    for collection in ln.Collection.filter().all():
        ...
        if not key.startswith("pert-gym/"):
            continue
        matches = list(collection.artifacts.filter(key=target_key).only("uid", "key").all())
        if matches:
            result[key] = {
                "uid": str(collection.uid), "hash": str(collection.hash),
...
def verify_current(ln: Any, source: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    obs_curated = str(obs_artifact.description).startswith(f"{TASK_ID}: source-exhaustive GSE150062 OBS")
    var_curated = str(var_artifact.description).startswith(f"{TASK_ID}: source-exhaustive GSE150062 VAR")
    ...
    if not var_curated and str(var_artifact.uid) != EXPECTED_PREWRITE_VAR_UID:
    ...
    obs_completed = all(
        ...
        for field in ("dataset", "cell_id", "assay", "modality", "perturbation", "guide_sequence", "is_control", "timepoint", "trajectory_id", "n_counts", "n_genes", "pct_mito", "pct_ribo", "is_low_quality")
    )
    return {
        ...
        "var_before": artifact_identity(var_artifact), "source_join": join_receipt,
        "source_manifest_sha256": sha256_file(SOURCE_MANIFEST_PATH),
        "canonical_field_dispositions": dispositions,
        "obs_completed": obs_completed, "var_verdict": var_verdict,
        "already_curated_obs": obs_curated, "already_curated_var": var_curated,
        "curated_obs": curated_obs, "curated_var": curated_var,
...
def publish(ln: Any, result: dict[str, Any], helper_sha256: str) -> dict[str, list[Any]]:
    ...
    if not result["already_curated_var"]:
        ...
        var = ln.Artifact.from_dataframe(
            ...
            description=f"{TASK_ID}: source-exhaustive GSE150062 VAR; exact 60497-feature Wu unified-reference identity and human organism; 44025 unique standard ENSG, 16401 source-native LH retained without fabrication",
        ).save()
...
def main() -> int:
    ...
    dataset_e2e = {
        "physical_member_count": 1, "matrix_axis_parity": True,
        "obs_completed": bool(final["obs_completed"]),
        "var_ensembl_species_completed": bool(final["var_verdict"]["var_ensembl_species_completed"]),
        "versioned_canonical_membership_preserved": True,
    }
    dataset_e2e["complete"] = all(dataset_e2e.values())
    dataset_e2e["status"] = "complete" if dataset_e2e["complete"] else "blocked_var_source_identifier_boundary"
    receipt = {
        "format": "pert-gym.real-dataset-obs-var-curation/v3", "task_id": TASK_ID,
        "real_dataset_id": REAL_DATASET_ID, "dataset_id": PREFIX, "status": "PASS", "mode": mode,
        "helper_sha256": helper_sha256, "source_manifest_sha256": sha256_file(SOURCE_MANIFEST_PATH),
        ...
        "dataset_e2e_v3": dataset_e2e, "collections": collections_after,
        "writes": {
