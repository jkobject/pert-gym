...
SOURCE_GENERATION_URI = (
    ...
    "20260717T045017Z-8821ce13/raw/perturbase.h5ad#1784263863449368"
)
...
def inspect_source(
    path: Path, baseline: pd.DataFrame, var: pd.DataFrame
) -> dict[str, Any]:
    with h5py.File(path, "r") as handle:
        payload = read_elem(handle)
    if "extend_61" not in payload:
        raise AssertionError("PerturBase extend_61 source member absent")
    member = payload["extend_61"]
    ...
    if nnz != 15_679_469 or not np.isclose(
        total, 9_958_565.121656165, rtol=0, atol=1e-6
    ):
...
def curate_obs(
    ...
    set_field(
        curated,
        "perturbation_library",
        "full 61-element developmentally critical TF ORF library plus mCherry control",
        "present",
        "paper Table S1 and Table S2",
    )
    set_field(
        curated,
        "guide_sequence",
...
def is_authorized_revision(artifact: Any, baseline_uid: str, description: str) -> bool:
    uid = str(artifact.uid)
    return (
        uid != baseline_uid
...
def prepare(ln: Any, manifest: dict[str, Any], *, full_source: bool) -> dict[str, Any]:
    ...
    if full_source:
        ...
        source_receipt = inspect_source(source_path, baseline, var)
    ...
    obs_curated = is_authorized_revision(latest_obs, BASELINE_OBS_UID, obs_description)
    var_curated = is_authorized_revision(latest_var, BASELINE_VAR_UID, var_description)
    if str(latest_obs.uid) != BASELINE_OBS_UID and not obs_curated:
        raise AssertionError(f"foreign OBS revision after baseline: {latest_obs.uid}")
    if str(latest_var.uid) != BASELINE_VAR_UID and not var_curated:
...
def ensure_successor_collection(
    ...
    *,
    ...
    successor_key = "pert-gym/additions/20260730-gse107185-e2e"
