...
| SCP/BioStudies/organoid candidates | SCP auth is partially recovered; SCP1467 heart is ingested | logged-in Chrome/computer-use recovered and byte-verified `SCP1467` expression/metadata plus the `SCP499` early-bud matrix under `gs://scperturb/pert-gym/staging/browser_auth_scp/2026-06-22/`. `SCP1467` now has verified triplets under `temporal_pretraining/scp1467_drosophila_embryonic_heart/` with raw counts as canonical X and normalized expression as `X_normalized_expression.h5ad`. `SCP499` still needs tiny sidecars; `SCP3301`, `SCP211`, and `SCP282` need matrix-family/representation selection before staging larger multi-file payloads. Older headless HTTP 401 probes are historical, not terminal blockers | current temporal batch artifacts under `artifacts/schema_audit/`; `temporal_scp_browser_auth_continuation_t1454d364_20260623.{md,json}`; `temporal_scp1467_heart_ingestion_20260623.{md,json}`; `temporal_t32_zebrafish_regen_scp_20260622.md` |
