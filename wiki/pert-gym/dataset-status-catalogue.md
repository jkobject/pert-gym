...
| SCP/BioStudies/organoid candidates | source-blocked unless processed matrices are recovered | some candidates need auth or only expose raw/unprocessed data; do not invent processed matrices. T32 zebrafish regeneration rows (`SCP1549`, `SCP1674`, `SCP1973`) expose SCP expression matrices/metadata and have no exact Lamin duplicate hits, but all download URL probes returned HTTP 401, so authenticated/session-aware SCP export is required | current temporal batch artifacts under `artifacts/schema_audit/`; `temporal_t32_zebrafish_regen_scp_20260622.md` |
