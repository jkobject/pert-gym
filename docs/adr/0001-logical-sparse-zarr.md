...
frame hash, and schema fingerprint. A reader verifies those hashes once and
reuses that one DataFrame for every yielded block; it never requires a
per-subdataset var artifact.

`pert_gym.logical_dataset.open_logical_dataset()` is the public compatibility
boundary. Opening a local manifest, remote promotion marker, or legacy triplet
reads metadata only. `iter_blocks(blocks=..., rows=...)` materializes one
...
example and `tools/smoke_logical_loader_lamin.py` for the read-only tiny Lamin
smoke.
