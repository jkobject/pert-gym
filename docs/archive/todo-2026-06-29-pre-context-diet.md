...
_Last updated: 2026-06-26 22:55 CEST — CTO board reconciliation (dependency-unblock audit + model-ready gate)._
...
- `docs/project/` — detailed internal/project knowledge:
  - `docs/project/current-status.md` — canonical latest status, count vocabulary,
    validation commands, temporal/model benchmark summaries, and blockers.
  - `docs/README.md` — wiki table of contents.
  - `docs/pert_gym_schema.md` — agent-readable schema summary.
  - `docs/project/harmonization-roadmap.md` — phase roadmap.
  - `docs/project/lamin-audit-and-branch-model.md` — branch/counting model.
  - `docs/project/dataset-modalities.md` — modality decisions.
  - `docs/project/deduplication-policy.md` — duplicate/subduplicate gate.

## Current source-of-truth status

Use `docs/project/current-status.md` for detailed numbers. Short version:
...
## CTO board reconciliation — 2026-06-29

_Updated from 2026-06-26. Board still shows stale `blocked` status for cards whose dependencies are `done`. Live board: `done=291`, `blocked=14`, `todo=8` (unchanged — re-status pending)._

Live board check: `done=291`, `blocked=14`, `todo=8`. Several `blocked` cards are
**dependency-unblocked, not actually blocked** — their parents are now `done` and
the board simply has not been re-statused. Reclassification:

**Now dispatchable (blocker effectively lifted — re-status to `ready`/dispatch):**

- `t_0a1b8f92` — Tahoe-100M streaming obs_uuid rewrite (14 plate obs artifacts).
  Parents `t_d4dd1cda`, `t_47b770a1`, `t_dd9f81b9` are all `done`. This is the
  last obs_uuid lane after non-PRISM (`t_47b770a1`), VIPerturb/Arc (`t_ac61bf4f`),
  and Broad PRISM (`t_f097b33c`), all `done`.
- `t_6ebac53a` — PRISM canonical obs_uuid rewrite (925 members / 902 chunks). Its
  acceptance precondition ("do not start until `t_dd9f81b9` + prior lanes done") is
  now satisfied. High-volume lane; run via the shared `add_obs_identity` helper.
- `t_56a127e9` — normalize canonical var to ENSEMBL/stable IDs. Parent audit
  (`organism_var_ensembl_cleanup_plan_20260624`) is complete: 75 datasets already
  ENSEMBL-like, 37 need symbol→ID mapping, 7 opaque, 1 broken-triplet skip.
- `t_62979e67` — 40 residual stale X→var links. Dispatchable on the **safe
  branch-local path only** (create branch-local X artifacts pointing to latest
  `jkobject` normalized var); do NOT write Lamin main. A main-write policy gate is
  the only thing that would keep this blocked, and the safe path avoids it.
- `t_8d6c08e0` — STT0000071 zebrafish heart staging/probe. Blocker lifted by
  Jérémie's verified CNGB FTP path (non-TIFF GEM/TSV/readme payloads listable);
  scope is source/staging only.

**Source-resolver cards (bounded research, no Lamin writes — dispatchable):**
`t_2c7a3795` (STAPR), `t_7771ffdc` (Arabidopsis life-cycle), `t_acb4edf4`
(Arabidopsis STOmics). Their review gate `t_6e147381` waits on these producers.

**Genuinely still blocked (real missing input / resource):**

- `t_8131df1c` / `t_22409681` — PerturBase row113 (GSE216481). MORF/TF label_map
  recovered, but the component-specific **filtered-cell inclusion table/predicate**
  for `201218_RNA` and `210322_TFAtlas` is still missing. Real blocker.
- `t_05c6b625` (MOSTA pilot), `t_15aaaa4a` (sequential GCS staging),
  `t_bd4a9c61` (GSE326326 range extractor + MOSTA manifest) — resource/disk gated
  (~28 GiB free); keep as staged/streamed-only design work.
- `t_fedb900d` — LINCS L1000 Level5 bounded probe; low priority, can wait.

**`t_073c145e` (totest Tahoe obs_uuid) — NOT ready to dispatch.** It is the tester
gate for parent `t_0a1b8f92`, which is not `done` yet. Dispatch the parent first;
the tester (`t_073c145e`) and reviewer (`t_6b0cb845`) auto-follow.
_(Confirmed 2026-06-29: board still shows `t_0a1b8f92` as `blocked` not `done`. t_073c145e remains todo/waiting.)_

**Guard `t_845b2209` — correctly stays `blocked`.** Its target stale duplicate
`t_a0d75ec9` ("REVIEW pert-gym PR-DATA") is still sitting in `todo`, and the real
PR #8 review loop (`t_d938d151` → `t_f441acef` → `t_3f32bab9`, producer
`t_f997b416`) is fully `done`. The guard exists precisely to stop `t_a0d75ec9`
from auto-promoting. Do not unblock the guard; ideally close `t_a0d75ec9` as a
stale duplicate so the guard becomes unnecessary.

## Model-ready expansion gate — CTO analysis 2026-06-29

**Root cause confirmed:** 3 harmonization lanes block all 1055 non-v0 canonical members from promotion.
**Expansion path = unlock these 4 cards in order:**

1. `t_0a1b8f92` (Tahoe obs_uuid) → `t_073c145e` (tester) → `t_6b0cb845` (reviewer)
2. `t_6ebac53a` (PRISM obs_uuid, 925 members)
3. `t_56a127e9` (var ENSEMBL normalization, 37+7 datasets)
4. `t_62979e67` (40 stale X→var links, safe branch-local path)

All 4 are **dispatchable now** — dependencies satisfied, board status is stale. Once these 4 land + `t_2ebc0965` global validation passes, first promotion batch ≈ **102 high-quality members** (VIPerturbSeq 55 + scPerturb 33 + Tahoe-100M 14).

**3 priorities:**
- P1: Re-status `t_0a1b8f92`, `t_6ebac53a`, `t_56a127e9`, `t_62979e67` → `ready` and dispatch dev agents.
- P2: Run `t_97aa7c72` (aux obs_uuid audit) before global validation `t_2ebc0965`.
- P3: Pre-create model-ready v2 promotion card now, `blocked` on `t_2ebc0965`, to avoid post-validation delay.

---

## Model-ready expansion gate — why `model-ready/20260621` is still 1 member

`pert-gym/model-ready/20260621` holds exactly **1** tiny loader-smoked member
(`viperturb/vimentin_screen_chunk_smoke/chunk_0000`). The other 1055 canonical
members are `triplet-integrity-ok` but **not promotable yet** because model-ready
requires loadable triplets with clean, model-usable identity on three axes, and
all three harmonization lanes are exactly the cards reclassified above:

1. **obs identity** — `obs_uuid`/`original_obs_index` rollout. Done for
   non-PRISM/VIPerturb/Arc/Broad; **Tahoe (`t_0a1b8f92`) and PRISM (`t_6ebac53a`)
   lanes remain**, then global validation `t_2ebc0965` + review `t_df92d572`.
2. **var identity** — stable/ENSEMBL IDs (`t_56a127e9`); 37+7 datasets unresolved.
3. **link integrity** — 40 stale X→var links (`t_62979e67`).

Until these land, promotion is gated. Once they do, the cleanest non-PRISM
sources are the natural first promotion batch: VIPerturbSeq (55), scPerturb (33),
Tahoe-100M (14) ≈ **102 high-quality members** to loader-smoke and promote into a
new `pert-gym/model-ready/<date>` collection. That is the lever that lets the
scGEN/GEARS/scPRAM/CPA benchmarks stop relying on tiny/synthetic fallbacks.
...
- P6A-Mac — T-cell GWPS streaming/chunked ingestion path completed enough to be
  recorded in `docs/project/current-status.md`.
...
- P6B-Mac (`t_d61b0ed9`) — refreshed 2026-06-23 in
  `artifacts/schema_audit/post_prism_huge_dataset_plan_refresh_20260623.md`.
  XAtlas/Orion remains missing but is no longer blocked solely by local scratch:
  mounted-backed read prototypes through `/Users/jkobject/mnt/gcs/scperturb`
  succeeded. Do not start Lamin writes until current huge ingestion is idle and a
  patched/reviewed `connect_pertdata()` XAtlas chunker exists. Do not prioritize
  it ahead of the four-item order above.
...
- T15-Mac (`t_625ca1ff`) resolved SCP1290/SCP3301/SCP1467 public manifests. SCP
  payload endpoints still return HTTP 401 without an authenticated SCP session,
  but SCP1290 has a bounded GEO scRNA alternate (`GSE153162`): 20 per-sample 10x
  H5 triplets were ingested and verified under
  ...
  SCP3301/GSE315712 is deferred as a 33 GB staged/chunked converter task, and
  SCP1467 remains blocked on authenticated SCP export or an alternate expression
  mirror. Artifacts:
  `artifacts/schema_audit/temporal_t15_scp_gse153162_status_20260622.{md,json}`
  and `artifacts/scripts/ingest_temporal_t15_scp_gse153162_20260622.py`.
- T16-Mac (`t_43b90af5`, `t_9560a74b`, `t_4451e626`) consumed the
  manually/browser-staged SCP batch from
  `gs://scperturb/pert-gym/staging/manual_downloads/2026-06-23/` and completed the
  contract-valid pieces. `SCP1469` is verified as
  `temporal_pretraining/scp1469_hand_gfp_drosophila_embryos` (`34,203 × 10,686`,
  36,561,974 sparse TSV nonzeros; cluster Category/X/Y sidecar only, because
  `Hrt_singletmetadata.txt` was not staged, so cell-level developmental phase is
  explicitly `unknown_from_staged_payload`). `SCP1570` is verified as four
  same-prefix HH-stage triplets: `hh4_pro` (`2,064 × 12,965`, 3,829,781 nnz),
  `hh5` (`6,363 × 16,901`, 21,539,631 nnz), `hh6` (`4,294 × 15,652`, 13,812,014
  nnz), and `hh7` (`6,588 × 24,356`, 16,066,722 nnz). The HH7 row uses the
  recovered browser-auth `HH7_matrix.mtx.gz` + `HH7_features.tsv.gz` +
  `HH7_barcodes.tsv.gz` family; the older top-level `HH7_expression_matrix.mtx.gz`
  (`15,656 × 6,113`) remains excluded because no matching barcode sidecar was
  recovered for that matrix family. SCP1846 has been streamed from the 7.29 GB /
  528,649,855-nnz MatrixMarket without `mmread` and verified as 130 same-prefix
  chunks under `temporal_pretraining/scp1846_pten_socs3_retinal_ganglion_regeneration/`
  (`129,441 × 23,308`; final chunk 441 cells; obs→X→var links and payloads OK).
  Artifacts:
  `artifacts/schema_audit/temporal_t16_scp_staged_ingestion_20260623.{md,json}`,
  `artifacts/schema_audit/temporal_t16_scp1570_hh5_hh7_ingestion_20260623.{md,json}`,
  `artifacts/schema_audit/temporal_t16_scp1570_hh5_hh7_residual_audit_20260623.{md,json}`,
  `artifacts/schema_audit/temporal_t16_scp1846_stream_ingestion_20260623.{md,json}`,
  `artifacts/schema_audit/temporal_t16_scp1846_light_verification_20260623.{md,json}`,
  `artifacts/scripts/ingest_temporal_t16_scp_staged_20260623.py`,
  `artifacts/scripts/ingest_temporal_t16_scp1570_hh5_hh7_residual_20260623.py`, and
  `artifacts/scripts/ingest_temporal_t16_scp1846_stream_mtx_20260623.py`.
...
Current blockers / live corrections as of 2026-06-26:

- T29-Mac (`t_22409681` plus row113 continuations) — row 115/GSE142078 is ingested and verified as `temporal_pretraining/perturbase/gse142078_luhmes_crispri_day8` (`8,843 × 33,694`, obs→X→var links/payloads OK; PerturBase 7,684-cell discrepancy preserved in obs `qc_note`). Row 114/GSE156170 is `excluded_with_reason` from the active path by user decision due perturbation-label/QC ambiguity; preserve the provenance note but do not reopen it as an active blocker. Row 113/GSE216481 now has recovered MORF/TFmap label mapping from Data S1 / Elsevier CDN (`gse216481_morf_supplement_validation_20260625.*`, `gse216481_morf_data_s1_label_map_20260625.tsv`). The remaining missing input before canonical write is the component-specific filtered-cell inclusion table/predicate for `201218_RNA` and `210322_TFAtlas`, not the ORF/TF-symbol map. Artifacts: `artifacts/schema_audit/gse216481_morf_supplement_validation_20260625.md`, `artifacts/schema_audit/temporal_t29_gse216481_row113_metadata_contract_20260623.md`, `artifacts/scripts/validate_temporal_perturbase_t29_gse216481_contract_20260623.py`.
- SCP browser-auth correction — do not list `SCP1467`, `SCP211`, `SCP282`, `SCP3301`, or `SCP499` as generic “needs download” blockers without first checking their 2026-06-23 ingestion/staging artifacts. Live Kanban shows follow-up cards completed for these.
- STT0000071 correction — old `is_download=false/files=[]` source probe is stale. User provided reachable FTP path `https://ftp.cngb.org/pub/stomics/STT0000071/Analysis/STSA0000734/STTS0001152/`; live check lists `T1_C1.70.gem.gz`, `T1_C1.70.tsv.gz`, `T1_C1.gem.gz`, readmes/event tar, and TIFFs. Next card should stage/inspect GEM/TSV/readme only; skip TIFFs.
- STAPR — still needs action: inspect the dynamic STAPR Downloads/API in browser, extract concrete processed matrix/spatial metadata URLs for the 8 planarian regeneration timepoints, then stage a small/bounded expression+metadata pilot before Lamin writes.
- Arabidopsis life-cycle — still needs action: resolve exact repository/accessions/download links from `https://travislee.science/` / paper `PMC12416547` for processed snRNA/spatial matrices.
- Arabidopsis STOmics — still needs action: identify exact CNGB/STOmics STDS/project accession; the catalogue still only has a generic datasets URL.
...
- T33 (`t_a13812f2`) — axolotl regeneration batch remains ingestion-blocked after
  ...
  row 136 `SCP499` exposes `EB.matrix.txt.gz` (40.4 MB) plus small sidecars and
  should be retried via logged-in Chrome/computer-use SCP export rather than
  treated as terminally blocked by old headless HTTP 401 probes. Artifact:
...
- T27 (`t_e004636c`) — SCP components should be retried via authenticated
  Chrome/computer-use: `SCP211` exposes 31 public API file records including
  expression/matrix/metadata files. Non-SCP residuals remain source-gated:
  `SCP3697` exposed no study files in the old probe and `GSE293573` exposed only
  GEO/SRA metadata with no processed supplementary matrix. Artifact:
  `artifacts/schema_audit/temporal_t27_organoid_public_batch_20260622.md`.
- T28 (`t_40508832`) — SCP282 should be retried via authenticated Chrome/computer-use
  because known 3/6-month expression files exist. Non-SCP residuals remain
  source-gated: `GSE269572` has no GEO supplementary processed matrix and
  `GSE329346` exposes only a bulk RNA-seq gene-expression supplement, not scRNA.
  Artifact:
...
- T32 (`t_d2e1109a` + residual `t_6570390a`) — ingested/verified from the
  2026-06-23 manual GCS staging. `SCP1674` regenerating fin osteoblasts is a
  same-prefix triplet at
  `temporal_pretraining/scp1674_zebrafish_fin_regeneration_osteoblasts`
  (`6,262 × 17,873`, raw counts, reg1–reg4 labels, no explicit control in staged
  metadata). `SCP1973` zebrafish retina regeneration is
  `temporal_pretraining/scp1973_zebrafish_retina_regeneration`
  (`11,690 × 20,726`, raw counts, uninjured/44 hpl/4 dpl/6 dpl labels, 2,243
  controls). Residual `SCP1549` beta-cell regeneration was converted from 5
  staged Seurat/RDS objects with Rscript+Seurat RNA counts and ingested as
  `temporal_pretraining/scp1549_zebrafish_beta_cell_regeneration`
  (`13,507 × 16,144`, raw counts, Control/0/2/7/14 dpa labels, 1,481 controls).
  obs→X→var links and X payload existence verified on branch `jkobject` for all
  three. Artifacts:
  `artifacts/schema_audit/temporal_t32_zebrafish_regen_ingestion_20260623.md`,
  `artifacts/schema_audit/scp1549_beta_cell_regeneration_ingestion_20260623.md`.
...
- DOC2 (`t_fc44edcc`) — completed `CLAUDE.md` as the concise agent operating
  guide.
- DOC4/DOC5 — cleaned human-facing and detailed documentation pages;
  use the linked canonical pages as detail, not this mirror.
...
2. Use the accepted public façade / agent boot / durable docs / current TODO split as the stable navigation
...
4. Route the patched XAtlas/Orion chunker + HCT116 smoke after current huge
   ingestion is idle. P6B-Mac refreshed the strategy in
   `artifacts/schema_audit/post_prism_huge_dataset_plan_refresh_20260623.md`:
   mount-backed reads work, but Lamin writes still need single-writer gating and
   a reviewed `connect_pertdata()` chunker.
5. Promote additional model-ready subsets from `triplet-integrity-ok` members.
6. Re-run/read the canonical status checks in `docs/project/current-status.md`
